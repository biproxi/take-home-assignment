import React from "react";
import Image from "next/image";
import Link from "next/link";
import { useRouter } from "next/router";

// The Navbar component is a simple TSX component that causes a navbar to render at the top of each page.  The Navbar houses links to the home page, to the display page (reviews), and to the modify page (contribute).


function Navbar() {
  const router = useRouter();
  return (
    <div>
      <nav className="bg-gray-800">
        <div className="max-w-7xl mx-1 px-4 sm:px-6 lg:px-8 trial">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <Link href="/" >
                  <Image
                    className="website-logo"
                    src="data:image/png;base64,"
                    alt="Yay-Nay Icon"
                    width={0}
                    height={0}
                  />
                </Link>
              </div>
              <div className="hidden md:block">
                <div className="ml-10 flex items-baseline space-x-4">
                  <Link
                    href="/"
                    className={`hover:bg-gray-700 px-3 py-2 rounded-md text-lg font-medium ${router.asPath === "/" ? "text-white" : "text-yellow-300"}`}
                  >
                    Yay-Nay Movie Reviews
                  </Link>

                  <Link
                    href="/display"
                    className={`hover:bg-gray-700 px-3 py-2 rounded-md text-lg font-medium ${router.asPath === "/display" ? "text-white" : "text-yellow-300"}`}
                  >
                    Reviews
                  </Link>

                  <Link
                    href="/modify"
                    className={`hover:bg-gray-700 px-3 py-2 rounded-md text-lg font-medium ${router.asPath === "/modify" ? "text-white" : "text-yellow-300"}`}
                  >
                    Contribute
                  </Link>
                </div>
              </div>
            </div>
          </div>
        </div>
      </nav>
    </div>
  );
}

export default Navbar
