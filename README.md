## Notes
- You will need to run `docker compose -f ./docker-stack.ym up -d db phpmyadmin` to get the db working first (Or `make docker-build`) 
- To login to phpmyadmin go to localhost:8080 and use username `root` and password `examplePassword`
- There is a test account ready to go to with userName `testUser` and password `password`
- `make run` seems to only sometimes work and _A lot_ of debugging has yielded no answers for me
- `make build` and `make docker-build` work great!
